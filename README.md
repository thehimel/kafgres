...
### Download Kafka Service Credentials

- Select the service.
- Go to `Overview` | `Connection information` | `Kafka`.
- Download the following files:
  - Access Key (`service.key`)
  - Access Certificate (`service.cert`)
  - CA Certificate (`ca.pem`)
- Store these files in a directory.
...
- Test the insertion that inserts a data generated by `Faker` with `python src/manage.py insert`.

### Observation

- Upon running the producer, data stream will be sent with an interval to the Kafka broker.
  - It will log the data sent to the server.
- Upon running the consumer, data stream will be received from the Kafka broker and inserted to the PostgreSQL server.
  - It will log the data received from Kafka broker and the data inserted to the PostgreSQL server.
